...
2. In the left-hand menu, choose **Domains** ↁE**Registered domains** ↁE**Register domain**.
...
**Figure 5.10.1**  ERegistering a new domain on Amazon Route 53.
...
3. You'll see 2 default records of type **NS** (Name Server) and **SOA**  Ethere's no need to edit these two records.
...
**Figure 5.10.2**  EHosted Zone with the default NS/SOA records.

4. The record pointing to Amplify (type **CNAME** or **A - Alias**) will be **automatically added by AWS Amplify** in the next step (Step 3) when you attach the custom domain  Eyou don't need to create it manually here unless Amplify requires domain verification via a separate CNAME record.
...
2. In the left-hand menu, choose **App settings** ↁE**Domain management**.
3. Click **Add domain**.
...
**Figure 5.10.3**  EAttaching a custom domain to the Amplify App.
