...
**Figure 5.7.1**  EImport the `openapi.yml` file to create a REST API.

API Gateway will automatically create an API named `SmartCV API` with all resources and methods.

![Created API](/images/5-Workshop/5.7-APIGateway/5.42.png)
**Figure 5.7.2**  EThe `SmartCV API` is automatically created with full resources/methods.
...
![Configure Authorizer](/images/5-Workshop/5.7-APIGateway/5.43.png)
**Figure 5.7.3**  EAttach the `cognitoAuth` Cognito Authorizer to the `smartcv-users` User pool.
...
![Enable CORS](/images/5-Workshop/5.7-APIGateway/5.44.png)
**Figure 5.7.4**  EEnable CORS for each resource (applications, insights, settings, notes, resumes).
...
2. Click **Edit integration** (or *Integration Request* ↁE*Edit*).
3. **Integration type:** *Lambda function*.
4. **Use Lambda Proxy integration:** this must be **Enabled (Checked)**.
5. **Lambda function:** type the corresponding function name (e.g., `/applications` ↁE`smartcv-applications`).
6. Click **Save** and grant permission when prompted.

Repeat this for all Resources: `/insights` ↁE`smartcv-insights`, `/settings` ↁE`smartcv-settings`, `/notes` ↁE`smartcv-notes`, `/resumes` ↁE`smartcv-applications`.

![Lambda Proxy Integration](/images/5-Workshop/5.7-APIGateway/5.45.png)
**Figure 5.7.5**  EIntegrate the Method with the corresponding Lambda function (Lambda Proxy Integration).
...
4. Click **Deploy**.
5. After successful deployment, you will see an **Invoke URL** (e.g., `https://xxxxxx.execute-api.ap-southeast-1.amazonaws.com/v1`). Copy this URL  Eit will serve as the Backend URL for the Frontend.

![Deploy successful, get Invoke URL](/images/5-Workshop/5.7-APIGateway/5.46.png)
**Figure 5.7.6**  EDeploy API to stage `v1` and get the Invoke URL for the Frontend.