...
**Figure 5.12.1**  EStructured JSON log displayed on CloudWatch Logs Insights.
...
- **`Tracing.ACTIVE`**: integrates **AWS X-Ray**, allowing you to trace the detailed path of a request across multiple services (API Gateway ↁELambda ↁEDynamoDB/Bedrock), which is useful for debugging performance issues or errors.

![Configuring logRetention and Tracing in CDK](/images/5-Workshop/5.12-CloudWatch/image10.png)
**Figure 5.12.2**  EConfiguring `logRetention` and `tracing` in CDK.

#### Viewing Traces in AWS X-Ray

1. Access the **CloudWatch** service ↁE**X-Ray traces** (or open the **X-Ray** service directly).
...
**Figure 5.12.3**  EService Map on AWS X-Ray showing the request flow through API Gateway, Lambda, and downstream services.
