...
{{% notice note %}}
**Important update:** The current Bedrock interface now **automatically enables (auto-enable)** serverless foundation models  Eyou **do not need** to manually "Request model access" as required in older Console versions.
{{% /notice %}}
...
**Figure 5.11.1**  EThe Model access page showing models that are already automatically available for use.

#### Step 2: Select a Model in the Model Catalog

1. Still in the sidebar, select **Model catalog**.
2. Type **Nova Lite** into the search box and select the **Amazon Nova Lite** model  Ethis is the official AI model used for the SmartCV project.
...
**Figure 5.11.2**  EAmazon Nova Lite in the Model catalog.
...
**Figure 5.11.3**  EOpening Amazon Nova Lite in the Playground to test it.
...
**Figure 5.11.4**  EGranting Playground access permission.
...
**Figure 5.11.5**  ECreating a Short-term API Key in the Bedrock Console.

2. Copy the API key you just created  Eyou'll use it in the next step.
...
{{% notice warning %}}
**Do not commit the API key directly into source code** when pushing to a public Git repository. A safer approach is to store the API key in the Lambda's environment variables (section [5.5](../5.5-lambda/) ↁEEnvironment variables, e.g. adding a `BEDROCK_API_KEY` variable) and read it from `os.environ` inside `handler.py`, instead of hard-coding it directly in the file.
{{% /notice %}}
