...
{{% notice note %}}
**Cập nhật quan trọng:** Giao diện Bedrock hiện tại **đã tự động kích hoạt (auto-enable)** các serverless foundation model  Ebạn **không cần** thực hiện thao tác "Request model access" thủ công như các phiên bản Console cũ trước đây.
{{% /notice %}}
...
**Hình 5.11.1**  ETrang Model access hiển thịcác model đã tự động sẵn sàng sử dụng.

#### Bước 2: Chọn model trong Model Catalog

1. Vẫn ở sidebar, chọn **Model catalog**.
2. Gõ **Nova Lite** vào ô tìm kiếm và chọn model **Amazon Nova Lite**  Eđây là model AI chính thức được dùng cho dự án SmartCV.
...
**Hình 5.11.2**  EAmazon Nova Lite trong Model catalog.
...
**Hình 5.11.3**  EMở Amazon Nova Lite trong Playground đểthử nghiệm.

2. Nếu được yêu cầu, cấp quyền (**Grant permission**) đề tài khoản của bạn được sử dụng Playground.

![Cấp quyền sử dụng Playground](/images/5-Workshop/5.11-Bedrock/5.11.1.png)
**Hình 5.11.4**  ECấp quyền truy cập Playground.
...
1. Trong Bedrock Console, tạo **Short-term API keys** đểdùng cho việc gọi Bedrock từ code Lambda.

![Tạo Short-term API Key](/images/5-Workshop/5.11-Bedrock/image8.png)
**Hình 5.11.5**  ETạo Short-term API Key trong Bedrock Console.

2. Copy API Key vừa tạo  Ekey này sẽ được dùng ở bước tiếp theo.
...
3. Dán API Key vừa copy vào đúng vị trí khai báo client (theo hướng dẫn cụ thểtrong comment của code).

{{% notice warning %}}
**Không commit API Key trực tiếp vào source code** khi đưa lên Git công khai. Cách an toàn hơn là lưu API Key vào biến môi trường của Lambda (mục [5.5](../5.5-lambda/) ↁEEnvironment variables, ví dụ thêm biến `BEDROCK_API_KEY`) và đọc từ `os.environ` trong `handler.py`, thay vì hard-code trực tiếp trong file.
{{% /notice %}}
